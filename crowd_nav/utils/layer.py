...
from crowd_nav.utils.gcn_utils import scaled_Laplacian, cheb_polynomial
...
class cheb_conv(nn.Module):
    ...
    def __init__(self, in_features, out_features, K, device, bias=True):
        ...
        self.Theta = nn.ParameterList(
            [nn.Parameter(torch.randn(self.in_channels, self.out_channels).to(self.DEVICE)) for _ in range(K)])

    def forward(self, x, adj):
        """
        Chebyshev graph convolution operation
        :param x: (B, N, C] --> (batch_size, N, F_in, T)  [batch_size, node_num, channel]
        ...
        for k in range(self.K):
            ...
            # （b, F_in, N) * (N, N) --> (b, F_in, N) --> (b, N, F_in)
