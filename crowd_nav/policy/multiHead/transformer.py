...
from crowd_nav.policy.multiHead.position_embedding import SinusoidalPositionalEmbedding
from crowd_nav.policy.multiHead.multihead_attention import MultiheadAttention
import math
...
class TransformerEncoder(nn.Module):
    ...
    # embed_dim=20, num_heads=4, layers=2
    def __init__(self, embed_dim, num_heads, layers, attn_dropout=0.0, relu_dropout=0.0,
        ...
        for layer in range(layers):
            new_layer = TransformerEncoderLayer(embed_dim, num_heads=num_heads, attn_dropout=attn_dropout,
    ...
    def forward(self, x_in, x_in_k=None, x_in_v=None):
        ...
        # print(x_in.shape) # torch.Size([20, 2, 20])
        x = self.embed_scale * x_in

        if self.embed_positions is not None:
            pe = self.embed_positions(x_in.transpose(0, 1)[:, :, 0]).transpose(0, 1)  # Add positional embedding
            x += pe
...
class PositionalEncoding(nn.Module):
    ...
    def __init__(self, d_model=50, dropout=0.1, max_len=5000):
...
if __name__ == '__main__':
    encoder = TransformerEncoder(50, 5, 2)  # embed_dim, num_heads, layers
    po = PositionalEncoding()
    x = torch.rand(2, 5, 50).clone().detach()
    x = po(x)
    # print(x.shape)
