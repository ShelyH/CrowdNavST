...
def main():
    ...
    logging.info('kinematics: %s', policy_config.get('action_space', 'kinematics'))
    ...
    policy = policy_factory[args.policy](action_space, capacity, batchsize, in_mlp_dims,
                                         action_dims, value_dims, device)
    ...
    for episode in range(num_updates):
        time_frames_state = env.reset(phase='train')
        done = False
        tol_reward = 0
        ...
        while not done:
            ...
            tol_reward += reward
            policy.replay_pool.push(time_frames_state, action, reward, time_frames_state_, done)
            time_frames_state = time_frames_state_

        q1_loss, q2_loss, pi_loss, alpha_loss = policy.optimize(training_step)
        ...
        logging.info('episode:{}, reward:{}, memory size:{}, time:{}, info:{}'
                     .format(episode, ('%.2f' % tol_reward), policy.replay_pool.__len__(),
